...
const languages = options.lang ? [options.lang] : SUPPORTED_LANGUAGES;
...
async function main() {
  ...
  console.log(
    `Found ${usedKeys.size} unique translation keys in the codebase.`
  );
...
async function extractTranslationKeys() {
  ...
  for (const file of files) {
    ...
    let match;
    while ((match = EXTRACT_PATTERN.exec(content)) !== null) {
      keys.add(match[1]);
    }
    ...
    // Extract keys from {t('key')} pattern
    while ((match = EXTRACT_PATTERN_ALT.exec(content)) !== null) {
      keys.add(match[1]);
    }

    // Reset regex
    EXTRACT_PATTERN_ALT.lastIndex = 0;
  }
...
function reportResults(missingTranslations, unusedTranslations) {
  ...
  for (const lang of Object.keys(missingTranslations)) {
    ...
    if (missing.length > 0) {
      hasMissing = true;
      console.log(
        `\n❌ Missing translations for ${lang.toUpperCase()} (${
          missing.length
        }):`
      );
  ...
  for (const lang of Object.keys(unusedTranslations)) {
    ...
    if (unused.length > 0) {
      hasUnused = true;
      console.log(
        `\n⚠️  Unused translations for ${lang.toUpperCase()} (${
          unused.length
        }):`
      );
