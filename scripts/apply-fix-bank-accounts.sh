...
# Apply the fix-bank-accounts migration to the Supabase database
...
echo "Applying fix-bank-accounts migration..."

# Use curl to execute the SQL script
curl -X POST "${SUPABASE_URL}/rest/v1/rpc/exec_sql" \
  ...
  -d @- << EOF
{
  "query": "$(cat supabase/fix_bank_accounts.sql | tr '\n' ' ')"
}
EOF
